...
impl Matrix {
    ...
    fn validate(&self, tetromino: &Tetromino) -> TetrominoValidity {
        ...
        match tetromino.snap_to_matrix().iter().all(|Snapped { row, column }| {
            (1..(PLAYFIELD_ROWS as i8)).contains(&row)
                && (1..(PLAYFIELD_COLUMNS as i8)).contains(&column)
    ...
    fn solidify(&self, tetromino: Tetromino) -> Self {
        ...
        for position in tetromino.snap_to_matrix() {
...
impl Tetromino {
    ...
    fn snap_to_matrix(&self) -> [Snapped; 4] {
        let a = self.minoes.map(|mino| Snapped {
...
#[derive(EnumCount, EnumIter, Debug, Clone, Copy)]
enum TetrominoType {
    O,
    I,
    T,
...
impl TetrominoType {
    fn new(&self) -> Tetromino {
        ...
        match self {
            ...
            L => Tetromino {
                center: Center::new(5.0 - 1.0, 19.0 - 1.0),
                minoes: [
                    Mino::new(0.0, 0.0),
                    Mino::new(-1.0, 0.0),
                    Mino::new(1.0, 0.0),
...
impl Display for Tetris {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for mino_position in self.falling_tetromino.snap_to_matrix() {
